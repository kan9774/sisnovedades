                        ...
                        <tr wire:key="salida-{{ $salida->id }}">
                            ...
                            <td>
                                @if ($salida->vehiculo)
                                    ...
                                    @endif
                                @else
                                    <span class="text-danger"><i class="fas fa-exclamation-triangle"></i> Vehículo
                                        eliminado</span>
                                @endif
                            </td>
                            <td>{{ $salida->conductor ? $salida->conductor->nombre_visible : 'Conductor eliminado' }}
                            </td>
                            <td>
                            ...
                            <td>
                                @if ($salida->tiene_boleta)
                                    ...
                                    @if ($salida->boletaCierre)
                                        <br><small
                                            class="text-muted">{{ $salida->boletaCierre->fecha_entra->format('d/m/Y') }}</small>
                                    @else
                                        <small class="text-muted"> {{ $salida->guardia->date->format('d/m/Y') }}
                                        </small>
                                    @endif
                                @else
                                ...
                                @if ($guardia->status === 'open' && $puedeOperarGuardia && $salida->guardia_id === $guardia->id)
                                    <div class="d-flex justify-content-center">
                                        ...
                                        <button type="button" wire:click="eliminar({{ $salida->id }})"
                                            wire:confirm="¿Eliminar esta salida?" class="btn-ops btn-ops-danger btn-xs"
                                            title="Eliminar salida">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    </div>
                                @elseif ($salida->guardia_id !== $guardia->id)
                                    <span class="badge-ops badge-ops-light border"
                                        title="Salió en la guardia del {{ $salida->guardia->date->format('d/m/Y') }}">
                        ...
                        @foreach ($this->resumenCombustible as $resumen)
                            <tr class="font-weight-bold" style="background: #f8f9fa;">
                                <td colspan="6" class="text-right">
                                    TOTAL {{ $resumen->tipoCombustible->nombre ?? '' }}:
                                </td>
            ...
            <div class="ops-panel">
                <form wire:submit="guardar" class="ops-panel__form">
                    <div class="ops-panel__header">
                        ...
                        <button type="button" class="ops-panel__close" onclick="cerrarOpsPanel('modalSalida')"
                            title="Cerrar">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select wire:model.live="vehiculo_id"
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select wire:model="conductor_id"
                                            class="form-control @error('conductor_id') is-invalid @enderror">
                                            <option value="">Seleccionar...</option>
                                            @foreach ($this->conductores as $conductor)
                                                <option value="{{ $conductor->id }}">{{ $conductor->nombre_visible }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('conductor_id')
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <select wire:model.live="tipo_combustible_id"
                                            class="form-control @error('tipo_combustible_id') is-invalid @enderror">
                                            <option value="">Seleccionar...</option>
                                            @foreach ($this->tiposCombustible as $tipo)
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="number" min="0" wire:model.live="kms_sale"
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="number" min="0" wire:model.live="kms_entra"
                    ...
                    <div class="ops-panel__footer">
                        <button type="button" class="btn btn-outline-secondary"
                            onclick="cerrarOpsPanel('modalSalida')">Cancelar</button>
            ...
            <div class="ops-panel">
                <form wire:submit="guardarBoleta" class="ops-panel__form">
                    <div class="ops-panel__header">
                        ...
                        <button type="button" class="ops-panel__close" onclick="cerrarOpsPanel('modalBoletaCierre')"
                            title="Cerrar">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="date" wire:model.live="boleta_fecha_entra"
                                                class="form-control @error('boleta_fecha_entra') is-invalid @enderror">
                                            @error('boleta_fecha_entra')
                                                <span class="invalid-feedback d-block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="time" wire:model.live="boleta_hora_entra"
                                                class="form-control @error('boleta_hora_entra') is-invalid @enderror">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="number" min="0" wire:model.live="boleta_kms_entra"
                                                class="form-control @error('boleta_kms_entra') is-invalid @enderror">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" wire:model.live="boleta_observaciones"
                                                class="form-control @error('boleta_observaciones') is-invalid @enderror"
                                                maxlength="500">
                                            @error('boleta_observaciones')
                                                <span class="invalid-feedback d-block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    </div>
                                ...
                                @if ($boleta_kms_entra && $salida->kms_sale && $boleta_kms_entra > $salida->kms_sale)
                                    <div class="alert alert-success mt-3">
                                        ...
                                        @if ($salida->vehiculo && $salida->vehiculo->consumo_litros_por_km)
                                            <br>Litros estimados:
                                            <strong>{{ number_format(($boleta_kms_entra - $salida->kms_sale) * $salida->vehiculo->consumo_litros_por_km, 2) }}
                                                L</strong>
                                        @endif
                                    </div>
                                @endif
                    ...
                    <div class="ops-panel__footer">
                        <button type="button" class="btn btn-outline-secondary"
                            onclick="cerrarOpsPanel('modalBoletaCierre')">Cancelar</button>
                        <button type="submit" class="btn" wire:loading.attr="disabled"
                            wire:target="guardarBoleta"
                            style="background: linear-gradient(135deg, #FFD200 0%, #FBCB5B 100%) !important; color: #0B2545 !important; font-weight: 700; box-shadow: 0 2px 8px rgba(255, 210, 0, 0.35) !important; border: none;">
                            <span wire:loading.remove wire:target="guardarBoleta"><i class="fas fa-save"></i> Guardar
                                Boleta</span>
                            <span wire:loading wire:target="guardarBoleta"><i class="fas fa-spinner fa-spin"></i>
                                Guardando...</span>
                        </button>
                    </div>
                </form>
