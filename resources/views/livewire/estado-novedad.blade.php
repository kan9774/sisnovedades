<div @if ($this->novedad && in_array($this->novedad->estado_atencion, ['pendiente', 'visto'])) wire:poll.5s="refrescar" @endif>
    @if ($this->novedad)
        @if ($this->novedad->estado_atencion === 'pendiente')
            ...
            @endcan
        @elseif ($this->novedad->estado_atencion === 'visto')
            ...
            <small class="text-muted {{ $compacto ? 'd-block' : '' }}">
                {{ $this->novedad->tomadoPor->name ?? '—' }}
                {{ $compacto ? '' : 'el ' . optional($this->novedad->tomado_en)->format('d/m H:i') }}
            </small>
        @else
